package com.g4vrk.react.service;
...
public class ViolationManager {
    ...
    public ViolationManager(SolarAI plugin) {
        this.plugin = plugin;
        ...
        this.mlConsoleFormat = plugin.getConfig().getString(
                ...
                "[SolarAI] ML flag: %player% prob=%prob% VL=%vl%"
        );
        this.checkConsoleFormat = plugin.getConfig().getString(
                "alerts.console.check-format",
                "[SolarAI] Check flag: %player% check=%check% VL=%vl%"
        );
    ...
    public void handleViolation(LocalPlayer entity, double probability) {
        if (!enabled) return;
    ...
    public void handleLegacyViolation(LocalPlayer entity, String checkName) {
        if (!enabled) return;
    ...
    private void checkForPunishment(LocalPlayer entity, double currentVl) {
    ...
    private void executePunishment(LocalPlayer entity, String mode) {
        ...
        if ("INSTANT".equals(mode)) {
            ...
            commandToExecute = "solar punish " + entity.getName();
        }
    ...
    private void executeProgressivePunishment(LocalPlayer entity, double currentVl) {
        UUID uuid = entity.getUuid();
    ...
    private void sendAlert(LocalPlayer entity, double probability, double currentVl) {
        ...
        String finalMessage = ChatColor.translateAlternateColorCodes('&',
                message.replace("%player%", entity.getName())
                        ...
                        .replace("%vl%", vlString));
        String permission = plugin.getConfig().getString("alerts.permission", "solarai.alerts");
